...
frame2=cv2.imread("DC1002.jpg")

box1 =((128,384),(128,96),(512,96),(512,384)) 
box2 = ((640/3,480*2/3),(640/3,480/3),(640*2/3,480/3),(640*2/3,480*2/3))
signal = 4
c=1
...
def calculate_Distance(image_path,focalLength_value,dis): 
...
#    cv2.waitKey(300) 
    ...
    cv2.line(image,(center_x,center_y),(center_x,center_y),(0,0,255),2)

    rdis=dis
    if rdis>WARNING_DISTANCE:
       cv2.putText(image,"Warning! Distance is out of range",(10,20),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
    else:
       cv2.putText(image,"The target did not migrate",(10,20),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2)
    ...
    cv2.putText(image, "Distance initial position:%2fcm" %dis, (image.shape[1] - 600, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1) 
...
def get_dis(image_path1,image_path2):
    first_image = image_path1
    new_image = image_path2
    
 #   cv2.imshow("image", image) 
 #   cv2.waitKey(300) 
    marker1 = find_marker(first_image)
    marker2 = find_marker(new_image)
#    if marker1[0][0]<marker1[0][1]
    ...
    rdis=math.sqrt(rdis_x*rdis_x+rdis_y*rdis_y+rdis_z*rdis_z)

    print("dis_x:%2fcm"%abs(rdis_x))
    print("dis_y:%2fcm"%abs(rdis_y))
    print("dis_z:%2fcm"%abs(rdis_z))
    print("dis:%2fcm"%abs(rdis))
    return rdis

def bbOverlap(box,box1,box2):
    box0=box
    box_det=box1
    box_cen=box2
    if box0.all() == None:
        ...
        if (box_cen[1][0] <= box0[1][0] <= (box_cen[2][0]-(box0[2][0]-box0[1][0])))and box0[2][0] < box_cen[2][0]and box0[2][1]>box_cen[2][1]and box0[3][0]< box_cen[3][0]:
            #print("Object_x in the central area")
            if ((box_cen[0][1]-(box0[0][1]-box0[1][1])) >= box0[1][1] >= box_cen[1][1]):
                #print("The object is in the central area")
                signal = 1
            else:
                if (box_det[1][0] <= box0[1][0] <= (box_det[2][0]-(box0[2][0]-box0[1][0]))):
                    #print("Object_x in the detectable area")
                    if ((box_det[0][1]-(box0[0][1]-box0[1][1])) >= box0[1][1] >= box_det[1][1]):
                        #print("Object in the detectable area")
                        signal = 2
                    else:
                        #print("Object outside the field of view")
                        signal = 0
                else:
                    signal = 0
        else:
            if (box_det[1][0] <= box0[1][0] <= (box_det[2][0]-(box0[2][0]-box0[1][0])))and box0[2][0] < box_det[2][0]and box0[2][1]>box_det[2][1]and box0[3][0]< box_det[3][0]:
                #print("Object_x in the detectable area")
                if ((box_det[0][1]-(box0[0][1]-box0[1][1])) >= box0[1][1] >= box_det[1][1]):
                    #print("Object in the detectable area")
                    signal = 2
...
def calibration_camera(image_path):
    image_cal=image_path
    cv2.imshow("calibration_camera",image_cal)
    
    marker=find_marker(image_cal)
    #print(marker)
    center_x = np.int0(marker[0][0])
    center_y = np.int0(marker[0][1])
    box = cv2.boxPoints(marker)
    box =np.int0(box)
    #print(box)
    signal = bbOverlap(box,box1,box2)
    print(signal)
    '''
    if dis_cal>10:
       cv2.drawContours(image_cal, [box], -1, (0, 0, 255), 2) 
       cv2.putText(image_cal,"Warning:Camera waiting for calibration",(10,60),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
       image_calibrade = image_cal
       signal = 0
    else:
       cv2.putText(image_cal,"System is ready",(10,60),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2)
       
    '''
    cv2.rectangle(image_cal, (128, 96), (512, 384), (255,0 ,0), 2)
    cv2.rectangle(image_cal, (640/3,480/3), (640*2/3,480*2/3), (255, 0, 0), 2)
    cv2.drawContours(image_cal, [box], -1, (0, 255, 0), 2)
    image_calibrade = image_cal
    cv2.imshow("calibration_camera",image_cal)
    return signal,image_calibrade
    #print(dis_cal)
...
if __name__ == "__main__": 
    ...
    while(1):
        ...
        new_photo= frame2
        if signal == 0:
           signal,image_cal = calibration_camera(frame2)
           cv2.imwrite("BD00002.jpg",frame2)
        else:
           focalLength = calculate_focalDistance(image_cal)
           dis = get_dis(image_cal,frame2)
           calculate_Distance(frame2,focalLength,dis)
           signal = 1
