...
def calculate_Distance(image_path,focalLength_value,dis_x,dis_y,dis_z): 
    ...
    if abs(rdis_x) <= WARNING_DISTANCE and abs(rdis_y) <= WARNING_DISTANCE and abs(rdis_z) <= WARNING_DISTANCE:
        cv2.putText(image,"System is normal",(10,40),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),1)
    else:
        if abs(rdis_x) > WARNING_DISTANCE:
             cv2.putText(image,"Warning : distance X exceeds threshold",(40,40),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),1)
        else:
            if abs(rdis_y) > WARNING_DISTANCE:
                cv2.putText(image,"Warning : distance Y exceeds threshold",(40,40),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),1)
            else:
                if abs(rdis_z) > WARNING_DISTANCE:
                    cv2.putText(image,"Warning : distance Z exceeds threshold",(40,40),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),1) 
...
if __name__ == "__main__": 
    ...
    for i in range(0,30) :
       ...
       marker1=get_dis(img_path1)
       marker2=get_dis(img_path2)
       ...
       calculate_Distance(img_path2,focalLength,rdis_x,rdis_y,rdis_z)
       cv2.waitKey(1000)
